...
class Speaker(object):
	...
	def __init__(self, *args,**kraws):
		super(Speaker, self).__init__()
		for name in kraws:
			setattr(self,name,kraws[name])
...
for i in range(200):
	...
	iname=names[index]
	postionN="职位"+str(index)
	index = random.randint(0,len(icons)-1)
	icon = icons[index]
	one = Speaker(id=id,name=iname,position=postionN,icon=icon);
	speaker.append(one)


res = [one.__dict__ for one in speaker]

path = os.path.join(os.getcwd(),"speakers.json")
